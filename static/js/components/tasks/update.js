            ...
            <form class="mt-3" id="updateTask" action="/api/tasks" method="PUT">
                ...
                <div class="form-row">
                    ...
                    <select class="form-select" name="priority" id="priority">
                        <option class="bg-light_coral hover:none" value="1" ${
                            task.priority === 1 ? 'selected' : ''
                        }>Urgent</option>
                        <option class="bg-max_yellow_red" value="2" ${
                            task.priority === 2 ? 'selected' : ''
                        }>Medium</option>
                        <option class="bg-cg_blue" value="3" ${
                ...
                <div class="form-row">
                    <p>Due Date</p>
                    <input class="form-input" value="${moment(
                        task.due_date
                    ).format(
                        'YYYY-MM-DD'
                    )}" type="date" name="due-date" required/>
                </div>
